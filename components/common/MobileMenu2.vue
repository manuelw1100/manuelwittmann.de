      ...
      <div class="content">
        <ul class="primary-menu">
          ...
          <li class="has-droupdown">
            <a
              @click="openDdMenu = !openDdMenu"
              :class="`nav-link ${openDdMenu ? 'open' : ''}`"
              href="#"
              >Pages</a
            >
            <ul
              :style="{ display: openDdMenu ? 'block' : 'none' }"
              :class="`submenu ${openDdMenu ? 'active' : ''}`"
            >
              <li v-for="(elm, i) in menuItemsElementPages" :key="i">
                <nuxt-link :to="elm.href">{{ elm.title }}</nuxt-link>
              </li>
            </ul>
          </li>
...
<script setup>
const openDdMenu = ref(false);
defineProps(["navItems"]);
import { menuItemsElementPages } from "~/data/menu";
